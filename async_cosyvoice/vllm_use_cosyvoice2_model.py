...
class CosyVoice3Model(nn.Module, SupportsLoRA, SupportsPP):
    ...
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = ""):
        ...
        self.llm_decoder = ParallelLMHead(self.speech_token_num,
                                      self.llm_output_size,
                                      quant_config=quant_config,
                                      prefix=maybe_prefix(
                                          prefix, "llm_decoder"))

        self.speech_embedding = torch.nn.Embedding(self.speech_token_num, self.llm_input_size)

        self.logits_processor = LogitsProcessor(self.speech_token_num)

        self.make_empty_intermediate_tensors = (
            self.model.make_empty_intermediate_tensors)

        # 定义特殊token常量
        self.llm_token_id_delta = torch.tensor(self.speech_token_num, dtype=torch.int32)
    ...
    def get_input_embeddings(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        mask = input_ids < self.llm_token_id_delta
        ...
        # 处理大于 delta 的  LLM token
        if (~flat_mask).any():
            llm_token_ids = flat_input_ids[~flat_mask]
            original_ids = llm_token_ids - self.llm_token_id_delta
            llm_embeds = self.model.get_input_embeddings(original_ids)
